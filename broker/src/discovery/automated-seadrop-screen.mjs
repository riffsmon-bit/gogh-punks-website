...
export const AUTOMATED_SCREEN_SCHEMA = "GOGH_AUTOMATED_SEADROP_SCREEN_V1";
...
function normalizeObservation(input, label, candidate, nowSeconds, maximumAgeSeconds) {
  ...
  if (value.collectionCodeHash !== COLLECTION_RUNTIME_CODE_HASH
    || value.collectionRuntimeLength !== 45 || value.seaDropCodeHash !== SEA_DROP_CODE_HASH) {
    fail("CODE_MISMATCH", `${label} code does not match the reviewed SeaDrop clone path`);
  }
...
export function screenAutomatedSeaDropCandidate(
  candidateInput,
  ...
  const evidence = {
    schema: AUTOMATED_SCREEN_SCHEMA,
    version: 1,
    chainId: 4663,
    ...
    safety: {
      ...
      exactCanonicalCloneRuntime: true,
