...
export const AUTOMATED_PROFILE_SCHEMA = "GOGH_AUTOMATED_SEADROP_PROFILE_V1";
export const AUTOMATED_LIVE_STATE_SCHEMA = "GOGH_AUTOMATED_SEADROP_LIVE_STATE_V1";
export const AUTOMATED_RUN_PLAN_SCHEMA = "GOGH_AUTOMATED_SEADROP_RUN_PLAN_V1";
...
export const COLLECTION_RUNTIME_CODE_HASH =
...
function normalizeProfile(input) {
  ...
  if (profile.schema !== AUTOMATED_PROFILE_SCHEMA || profile.version !== 1
    ...
    fail("INVALID_PROFILE", "profile must use the Robinhood automated SeaDrop V1 schema");
  }
  ...
  exactKeys(profile.infrastructure, [
    ...
    "cloneImplementationCodeHash", "collectionRuntimeCodeHash",
  ...
  const infrastructure = {
    ...
    collectionRuntimeCodeHash: hash(
      profile.infrastructure.collectionRuntimeCodeHash,
      "profile.infrastructure.collectionRuntimeCodeHash",
    ),
  };
  if (infrastructure.seaDrop !== SEA_DROP
    ...
    || infrastructure.collectionRuntimeCodeHash !== COLLECTION_RUNTIME_CODE_HASH) {
...
function normalizeLiveState(input, profile, nowSeconds) {
  ...
  if (live.schema !== AUTOMATED_LIVE_STATE_SCHEMA || live.chainId !== ROBINHOOD.chainId) {
    fail("INVALID_LIVE_STATE", "live state must use the automated Robinhood V1 schema");
  }
  ...
  if (address(live.owner, "liveState.owner") !== profile.punk.expectedOwner
    ...
    fail("IDENTITY_CHANGED", "owner, V2 Punk Account, or agent changed");
  }
  ...
  const targets = live.targets.map((target, index) => {
    ...
    seen.add(collection);
    if (target.collectionCodeHash !== COLLECTION_RUNTIME_CODE_HASH
      || target.collectionRuntimeLength !== 45) {
      fail("TARGET_INELIGIBLE", `${label} is not the reviewed canonical clone runtime`);
    }
...
export function buildAutomatedSeaDropRunPlan(profileInput, liveStateInput, options = {}) {
