CREATE TABLE IF NOT EXISTS broker_automation_v3_worker_runs (
  ...
  UNIQUE (release_commit, started_at, completed_at)
);

CREATE UNIQUE INDEX IF NOT EXISTS broker_automation_v3_worker_runs_transaction_idx
  ON broker_automation_v3_worker_runs (transaction_hash)
  WHERE transaction_hash IS NOT NULL;

CREATE INDEX IF NOT EXISTS broker_automation_v3_worker_runs_completed_idx
  ON broker_automation_v3_worker_runs (completed_at DESC);

INSERT INTO broker_automation_v3_worker_runs
  (release_commit, started_at, completed_at, status, submitted, punk_token_id,
   account_address, collection_address, transaction_hash, failure_code)
SELECT release_commit, started_at, completed_at, status, submitted, punk_token_id,
       account_address, collection_address, transaction_hash, failure_code
  FROM broker_automation_v3_worker_state
