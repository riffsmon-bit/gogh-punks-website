...
/// @title AutomatedSeaDropStudioPaidMintAdapter
/// @notice Exact-price native-token adapter for reviewed OpenSea Studio SeaDrop ERC-721 runtimes.
/// @dev The adapter accepts no opaque data, approvals, caller-selected target, recipient, quantity,
///      fee recipient, or slippage. The Punk policy remains responsible for the per-transaction,
///      daily, weekly, reserve, acquisition-count, collection, selector, and authorization limits.
contract AutomatedSeaDropStudioPaidMintAdapter is IGoghMarketplaceAdapter {
    ...
    bytes32 public immutable expectedCloneRuntimeCodeHash;
    bytes32 public immutable expectedStudioRuntimeCodeHash;

    error InvalidPinnedContract(address target);
    error InvalidPinnedHash(address target, bytes32 expected, bytes32 actual);
    error InvalidStudioRuntimeHash(bytes32 supplied);
    error UnreviewedCollectionRuntime(address target, bytes32 actual);
    error CollectionIsNotERC721(address target);
    ...
    error InvalidIntentPrice(uint256 expectedPrice, uint256 maxPrice);
    ...
    error PublicDropNotPaid(uint256 mintPrice);
    error PublicDropPriceChanged(uint256 expectedPrice, uint256 currentPrice);
    ...
    constructor(
        bytes32 seaDropCodeHash_,
        bytes32 cloneImplementationCodeHash_,
        bytes32 studioRuntimeCodeHash_
    ) {
        if (SEA_DROP.code.length == 0) {
            revert InvalidPinnedContract(SEA_DROP);
        }
        ...
        bytes32 cloneRuntimeHash = keccak256(_canonicalCloneRuntime());
        if (studioRuntimeCodeHash_ == bytes32(0) || studioRuntimeCodeHash_ == cloneRuntimeHash) {
            revert InvalidStudioRuntimeHash(studioRuntimeCodeHash_);
        }
        expectedSeaDropCodeHash = seaDropCodeHash_;
        expectedCloneImplementationCodeHash = cloneImplementationCodeHash_;
        expectedCloneRuntimeCodeHash = cloneRuntimeHash;
        expectedStudioRuntimeCodeHash = studioRuntimeCodeHash_;
    }
    ...
    function isReviewedCollectionRuntime(address collection) public view returns (bool) {
        bytes32 actual = collection.codehash;
        return collection.code.length != 0
            && (actual == expectedCloneRuntimeCodeHash || actual == expectedStudioRuntimeCodeHash);
    }
    ...
    function buildExecution(
        ...
        _requireReviewedCollection(intent.collection);

        IOpenSeaSeaDrop.PublicDrop memory drop =
            IOpenSeaSeaDrop(SEA_DROP).getPublicDrop(intent.collection);
        if (drop.mintPrice == 0) revert PublicDropNotPaid(drop.mintPrice);
        if (uint256(drop.mintPrice) != intent.expectedPrice) {
            revert PublicDropPriceChanged(intent.expectedPrice, drop.mintPrice);
        }
        ...
        execution.value = drop.mintPrice;
        execution.currency = address(0);
        execution.allowanceSpender = address(0);
        execution.allowanceAmount = 0;
        execution.paymentAmount = drop.mintPrice;
    ...
    function _validateIntent(
        ...
        if (intent.opportunityType != GoghBrokerTypes.OpportunityType.MINT) {
        ...
        if (intent.expectedPrice == 0 || intent.maxPrice != intent.expectedPrice) {
            revert InvalidIntentPrice(intent.expectedPrice, intent.maxPrice);
        }
    ...
    function _requireReviewedCollection(address collection) private view {
        bytes32 actual = collection.codehash;
        if (
            collection.code.length == 0
                || (actual != expectedCloneRuntimeCodeHash
                    && actual != expectedStudioRuntimeCodeHash)
        ) revert UnreviewedCollectionRuntime(collection, actual);
        try IERC165(collection).supportsInterface(ERC721_INTERFACE_ID) returns (bool supported) {
            if (!supported) revert CollectionIsNotERC721(collection);
        } catch {
            revert CollectionIsNotERC721(collection);
        }
    }
