...
/// @title AutomatedSeaDropStudioFreeMintAdapter
/// @notice Free-only adapter for reviewed OpenSea Studio SeaDrop ERC-721 runtime families.
/// @dev V3 expands V2 from the canonical clone to one separately pinned full ERC721SeaDrop
///      runtime. It never accepts an interface-only or caller-supplied runtime. Both reviewed
///      families start token IDs at one, so the next ID is currentTotalSupply + 1.
contract AutomatedSeaDropStudioFreeMintAdapter is IGoghMarketplaceAdapter {
