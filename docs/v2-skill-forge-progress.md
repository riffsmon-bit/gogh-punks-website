...
Latest approved product configuration: **seven maximum equipped skills**, rarity-based starting slots of 3 (top 5%), 2 (next band through top 25%) or 1 (remainder), and no rarity multiplier for sacrifices. One sacrifice grants one credit, spent on one slot OR one approved Tier-I skill. The local UI fixture now uses cap seven; old contract regression fixtures retain their independent cap-four test cases. The complete OpenSea rarity snapshot is **captured and verified: 4,295 Punks**; no real token bonus has been assigned. See [snapshot retrieval status](v2-rarity-snapshot-status.md). Latest combined JavaScript run: **86 passed**, including the pinned frozen-artifact regression. Solidity: **21 passed**, including 1,024 fuzz runs. Market Scout now also has authenticated live read evidence, not production approval.
