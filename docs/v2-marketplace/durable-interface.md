...
Claims re-run the existing core preparation against trusted current source/screen/policy/skill evidence, compare identity/account state/nonce/selection/runtime/fees, confirm the original anchor, and simulate the **stored original transaction** before the CAS. Refreshed calldata is never substituted. Database time independently rejects expired insertion and claim. A release change prevents claim. Only the committed CAS winner receives a transaction. Each insert/CAS checks out one pool connection, begins a transaction, sets `LOCAL synchronous_commit=on`, and checks `fsync=on`, `full_page_writes=on` and permanent journal/audit tables on that same connection before and after mutation. It awaits COMMIT before returning and always releases the connection. A lost commit acknowledgement withholds the payload; the original reservation may already be persisted and must be recovered. A lost HTTP response leaves `WALLET_REQUESTED` held.
