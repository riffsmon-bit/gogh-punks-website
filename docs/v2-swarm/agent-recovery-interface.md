...
await recovery.submit({expectedReview: displayedReview}); // immutable review actually shown to the owner
...
Methods return state snapshots. `submit({expectedReview})` requires the full immutable review displayed to the owner; missing snapshots reject with `AGENT_RECOVERY_REVIEW_REQUIRED`, and any normalized mismatch with the current PREPARED journal rejects with `AGENT_RECOVERY_REVIEW_CHANGED` under the Web Lock before API/preflight/wallet calls. Do not load a replacement review from storage at click time and treat it as approved. `getState()` includes `schema`, `status`, `review`, `transactionHash` and `receipt`. States: `EMPTY`, `PREPARED`, `WALLET_REQUESTED`, `SUBMITTED`, `CONFIRMED`, `REVERTED`, `REJECTED`, `CANCELLED`. Expired PREPARED remains inspectable but must be cancelled and reviewed again. UI should render the current action, source Agent address, owner destination, amount/token, maximum fee and expiry before enabling confirmation.
...
Restored CONFIRMED/REVERTED journals require a coherent transaction hash and receipt with matching hash, canonical-shaped block identity and success/revert status. These stored receipts are cached confirmation evidence, not a fresh RPC verification or proof of current NFT custody. The explicit refresh path rechecks chain receipts; current holdings require the separate collection custody reader. Nonstandard assets can emit standard-looking logs, so receipt/event verification does not prove the latest ownerOf state or perfect asset safety.

Security follow-up: the independent reviewer’s two-tab approval substitution proof now rejects before RPC or wallet work. The owner-displayed snapshot is copied before lock acquisition and compared in full under the lock. The corrected module passed the independent stale-approval proof plus the reviewer’s event-versus-custody characterization (temporary test runner only; reviewer-owned source unchanged). The earlier characterization accepting a terminal journal without receipt evidence is superseded by 18 new terminal-journal rejection/coherence cases. No production request was made for this correction.
