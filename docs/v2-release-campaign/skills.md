...
After a wallet request is reserved, only its original transaction can complete recovery. A wallet rejection or unknown result stays held rather than reopening the same wallet request. An administrator must resolve any pending wallet transaction before proceeding; the component does not fabricate a receipt from a rejection report. Review this conservative recovery behavior during the composed administrator acceptance before enabling the route for use.
