...
## Screenshot follow-up: unavailable review journal

The running preview returned `TRAINING_REVIEW_UNAVAILABLE` for all three fixtures while contract snapshots remained readable. Debugger inspection identified a latched `TRAINING_JOURNAL_UNAVAILABLE`. SQLite integrity and payload checks passed, and an independent read-only coordinator verified both recorded confirmed transactions against the then-running Anvil. The original triggering storage error was not retained, so its cause is unproven; low disk space alone is not a diagnosis.

The UI now keeps a fresh validated contract snapshot visible if recovery fails, with training, equipment, receipt/hash controls and research tools locked until recovery is verified. Owner/network changes still clear prior state. A journal-lock message distinguishes service maintenance from a retryable request failure. The coordinator now preserves the original journal exception as its internal cause; no raw internal errors or secrets are exposed in browser responses.

During diagnosis, debugger cleanup attempted an unsupported dynamic import and terminated the preview process. Its orphaned disposable Anvil subsequently exited too. The old SQLite journal remains untouched, but it is not a chain backup and cannot restore that session's canonical receipt history. No historical recovery is claimed. A fresh disposable fixture session was started at the same local URL; production assets and missions were untouched.

Added a process-local `resume` adapter and `suspend` hook for a STILL-RUNNING owned Anvil. Resume uses loopback only, checks chain/client/owner and existing journal deployment identity (genesis, addresses and code hashes), skips all deployment/seed transactions, validates recovery before listening, and never kills an attached chain. It is not exposed through HTTP and cannot recreate an exited chain. Durable chain checkpoints remain future work; the standard runner still creates temporary fixtures.

Verification: 80/80 targeted tests passed, including same-chain preview handover, retained confirmed receipts, lost-hash ambiguity, no resend, wrong-owner/deployment rejection and missing-journal rejection. Both journal-failure cases were rerun with assertions that the original cause survives. The desktop/mobile browser suite passed with a forced recovery outage, retained loadout/history and disabled actions. The replacement session was additionally smoke-tested through review/cancel and a journal-lock UI recovery; no training transaction was submitted by that smoke test.
