...
test("broker UI renders only allowlisted OpenSea display metadata", async () => {
  ...
  assert.match(client, /hostnames\.has\(url\.hostname\)/);
  assert.match(client, /OPENSEA_IMAGE_HOSTS = new Set\(\["i\.seadn\.io", "raw2\.seadn\.io"\]\)/);
  ...
  assert.match(
    headers,
    /img-src 'self' data: https:\/\/i\.seadn\.io https:\/\/raw2\.seadn\.io/,
  );
