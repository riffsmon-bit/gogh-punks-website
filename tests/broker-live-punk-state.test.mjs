...
test("Punk recommendations are collapsed and token metrics avoid nested-label styling", async () => {
  ...
  assert.match(brokerHtml, /data-ownership-state/);
  assert.match(brokerHtml, /Refresh My Punks/);
  assert.match(brokerHtml, /Turn your Gogh Punk into an autonomous art collector/);
  assert.doesNotMatch(brokerHtml, /data-scout-token-display/);
  assert.doesNotMatch(brokerHtml, /data-selected-punk-display|data-public-scout-token-display/);
  assert.doesNotMatch(brokerHtml, /data-live-summary/);
  ...
  assert.doesNotMatch(brokerHtml, /data-workspace-punk-picker|data-wizard-punks/);
