...
const CANONICAL = `0x${'5'.repeat(40)}`, SALT = `0x${'0'.repeat(64)}`, TX_HASH = `0x${'a'.repeat(64)}`;
...
function fixture({ created = true } = {}) {
  ...
  const event = (name, args, data, address = VAULT) => ({ address, topics: encodeEventTopics({ abi: ABI, eventName: name, args }), data,
    removed: false, transactionHash: TX_HASH, blockHash: word(101), blockNumber: '0x65' });
  function mine(review, { status = '0x1', confirmations = 12 } = {}) {
    ...
    state.tx.blockNumber = '0x65'; state.tx.blockHash = word(101);
    state.receipt = { from: OWNER, to: review.transaction.to, transactionHash: TX_HASH, blockNumber: '0x65', blockHash: word(101), status,
      gasUsed: hex(state.gas), effectiveGasPrice: hex(state.gasPrice), logs };
  }
