...
import {
  AUTOMATION_V2_DEPLOYMENT_ORDER,
  buildAutomatedSeaDropV2DeploymentManifestProposal,
  verifyAutomationV2SourceProvenance,
} from "../scripts/build-automated-seadrop-v2-deployment-manifest.mjs";
...
const names = AUTOMATION_V2_DEPLOYMENT_ORDER;
const broadcastNames = [
  "AutomatedSeaDropFreeMintAdapter", "BrokerPolicyModuleV2",
  "GoghPunkAccountV1", "GoghPunkAccountRegistryV2",
];
const addresses = {
  AutomatedSeaDropFreeMintAdapter: "0x70e8b0361b36bab3b398ab9dD0D9E370A70912b2",
  BrokerPolicyModuleV2: "0x4a80F21513C788884B4935edC27d119D059c0a9B",
  GoghPunkAccountV2: "0xa2f76E063f50178FCe534E50E40a6Daab83F4eAd",
  GoghPunkAccountRegistryV2: "0x00E7D2A869cc6a8f61A4cE11A66A8874db1F78e3",
};
const args = {
  AutomatedSeaDropFreeMintAdapter: [H("5"), H("d")],
  BrokerPolicyModuleV2: [GUARDIAN, ADAPTER_REGISTRY, addresses.AutomatedSeaDropFreeMintAdapter],
  GoghPunkAccountV2: [addresses.BrokerPolicyModuleV2, AGENT_REGISTRY, ADAPTER_REGISTRY],
  GoghPunkAccountRegistryV2: [addresses.GoghPunkAccountV2, ZERO_HASH],
};
args.AutomatedSeaDropFreeMintAdapter = [
...
function fixture(compiledArtifacts) {
  ...
  function resultForRead(address, functionName) {
    ...
    const table = {
      [addresses.AutomatedSeaDropFreeMintAdapter.toLowerCase()]: {
        expectedSeaDropCodeHash: args.AutomatedSeaDropFreeMintAdapter[0],
        expectedCloneImplementationCodeHash: args.AutomatedSeaDropFreeMintAdapter[1],
        expectedCollectionRuntimeCodeHash:
          "0xe3e252831cdd0c11e1327d04a57ddd9bfa11ef49d50edb524040d98bfb228bc4",
      },
      [addresses.BrokerPolicyModuleV2.toLowerCase()]: {
        owner: GUARDIAN, pendingOwner: ZERO, adapterRegistry: ADAPTER_REGISTRY,
        automatedSeaDropAdapter: addresses.AutomatedSeaDropFreeMintAdapter,
      },
      [addresses.GoghPunkAccountV2.toLowerCase()]: {
        GOGH_PUNKS: COLLECTION, ROBINHOOD_CHAIN_ID: 4663n,
        policyModule: addresses.BrokerPolicyModuleV2, agentRegistry: AGENT_REGISTRY,
        adapterRegistry: ADAPTER_REGISTRY,
      },
      [addresses.GoghPunkAccountRegistryV2.toLowerCase()]: {
        implementation: addresses.GoghPunkAccountV2, accountSalt: ZERO_HASH,
...
test("builds a fail-closed V2 deployment proposal with configuration still disabled", async () => {
  const input = fixture(await artifacts());
  const result = await buildAutomatedSeaDropV2DeploymentManifestProposal(input);
  ...
  assert.equal(result.manifest.contracts.GoghPunkAccountV2.receiptStatus, "SUCCESS");
...
test("rejects wrong runtime, constructor, provider, and critical bindings", async () => {
    ...
    (value) => { value.readEndpoints[1].client.readContract = async ({ functionName }) => (
      functionName === "owner" ? DEPLOYER
        : base.readEndpoints[0].client.readContract({
          address: addresses.BrokerPolicyModuleV2, functionName,
        })
    ); },
  ];
  for (const mutate of mutations) {
    ...
    await assert.rejects(
      buildAutomatedSeaDropV2DeploymentManifestProposal(value),
...
test("source provenance requires exact clean HEAD before and after offline build", async () => {
  ...
  const result = await verifyAutomationV2SourceProvenance({
    releaseGitCommit: RELEASE,
