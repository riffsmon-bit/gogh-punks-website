...
export const AUTOMATION_V3_DEPLOYMENT_ORDER = Object.freeze([
...
const COMPILER_INPUT_PATHS = Object.freeze([
  "contracts/src",
  "contracts/script/DeployAutomatedSeaDropV3.s.sol",
  "foundry.toml",
  "remappings.txt",
  "package.json",
  "package-lock.json",
...
const STUDIO_REFERENCE_COLLECTION = "0xC73Ee4987FDAd897e691EEccfa65C80Efb97f6f4";
const STUDIO_RUNTIME_HASH =
...
export class AutomationV3ManifestError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "AutomationV3ManifestError";
    this.code = code;
  }
}

function fail(code, message) {
  throw new AutomationV3ManifestError(code, message);
}
...
function parseBroadcast(artifact, compiled, releaseCommit) {
  ...
  if (value.chain !== CHAIN_ID || !Array.isArray(value.transactions)
    ...
    fail("INVALID_BROADCAST", "broadcast artifact is not the exact four-CREATE V3 run");
  }
  ...
  let deployer;
  const records = AUTOMATION_V3_DEPLOYMENT_ORDER.map((name, index) => {
...
async function verifyCriticalBindings(endpoints, blockNumber, addresses) {
  ...
  const bindings = {
    adapter: {
      expectedSeaDropCodeHash: await read(addresses.AutomatedSeaDropStudioFreeMintAdapter,
        "expectedSeaDropCodeHash", bytes32Output),
      expectedCloneImplementationCodeHash: await read(addresses.AutomatedSeaDropStudioFreeMintAdapter,
        "expectedCloneImplementationCodeHash", bytes32Output),
      expectedCloneRuntimeCodeHash: await read(addresses.AutomatedSeaDropStudioFreeMintAdapter,
        "expectedCloneRuntimeCodeHash", bytes32Output),
      expectedStudioRuntimeCodeHash: await read(addresses.AutomatedSeaDropStudioFreeMintAdapter,
        "expectedStudioRuntimeCodeHash", bytes32Output),
    },
    policy: {
      owner: await read(addresses.BrokerPolicyModuleV3, "owner"),
      pendingOwner: await read(addresses.BrokerPolicyModuleV3, "pendingOwner"),
      adapterRegistry: await read(addresses.BrokerPolicyModuleV3, "adapterRegistry"),
      automatedSeaDropAdapter: await read(addresses.BrokerPolicyModuleV3,
        "automatedSeaDropAdapter"),
    },
    accountImplementation: {
      collection: await read(addresses.GoghPunkAccountV3, "GOGH_PUNKS"),
      chainId: (await read(addresses.GoghPunkAccountV3, "ROBINHOOD_CHAIN_ID", uintOutput)).toString(),
      policyModule: await read(addresses.GoghPunkAccountV3, "policyModule"),
      agentRegistry: await read(addresses.GoghPunkAccountV3, "agentRegistry"),
      adapterRegistry: await read(addresses.GoghPunkAccountV3, "adapterRegistry"),
    },
    registry: {
      implementation: await read(addresses.GoghPunkAccountRegistryV3, "implementation"),
      accountSalt: await read(addresses.GoghPunkAccountRegistryV3, "accountSalt", bytes32Output),
      canonicalRegistry: await read(addresses.GoghPunkAccountRegistryV3, "canonicalRegistry"),
    },
  };
  if (hash(bindings.adapter.expectedSeaDropCodeHash, "SeaDrop immutable") !== SEA_DROP_HASH
    ...
    || hash(bindings.adapter.expectedCloneRuntimeCodeHash, "clone runtime immutable")
      !== COLLECTION_RUNTIME_HASH
    || hash(bindings.adapter.expectedStudioRuntimeCodeHash, "Studio runtime immutable")
      !== STUDIO_RUNTIME_HASH
    ...
    || address(bindings.policy.automatedSeaDropAdapter, "policy adapter")
      !== addresses.AutomatedSeaDropStudioFreeMintAdapter
    ...
    || address(bindings.accountImplementation.policyModule, "account policy")
      !== addresses.BrokerPolicyModuleV3
    ...
    fail("IMMUTABLE_BINDING_MISMATCH", "critical live V3 binding differs from the release");
  }
  return bindings;
}
...
export async function verifyAutomationV3SourceProvenance({
  releaseGitCommit,
...
export async function buildAutomatedSeaDropV3DeploymentManifestProposal(input) {
  plain(input, "input");
  ...
  const compiled = Object.fromEntries(AUTOMATION_V3_DEPLOYMENT_ORDER.map((name) => [
  ...
  const manifest = {
    schema: "GOGH_AUTOMATED_SEADROP_V3_DEPLOYMENT_MANIFEST",
    version: 1,
    status: "DEPLOYED",
    chainId: CHAIN_ID,
    gitCommit: releaseCommit,
    ...
    infrastructure: {
      ...
      cloneCollectionRuntimeCodeHash: COLLECTION_RUNTIME_HASH,
      studioReferenceCollection: getAddress(STUDIO_REFERENCE_COLLECTION),
      studioCollectionRuntimeCodeHash: STUDIO_RUNTIME_HASH,
    },
    configuration: {
    ...
    notes: "Immutable V3 deployment proposal from a clean release build and exact four-CREATE broadcast. Two distinct RPC provider domains agreed on final receipts, transaction inclusion, runtime code, and critical immutable/module bindings at one common confirmed block. Source verification remains NOT_SUBMITTED. No adapter registration, feature enablement, account activation, agent authorization, worker enablement, or mint is authorized by this proposal.",
  };
  return {
    schema: "GOGH_AUTOMATED_SEADROP_V3_DEPLOYMENT_MANIFEST_PROPOSAL_V1",
    proposalStatus: "AUTOMATION_V3_MANIFEST_PROPOSAL_SOURCE_VERIFICATION_PENDING",
    trustBindings: {
      chainId: CHAIN_ID,
      releaseGitCommit: releaseCommit,
      foundryArtifactCommit: broadcast.foundryCommit,
      sourceProvenance: provenance,
      deploymentOrder: AUTOMATION_V3_DEPLOYMENT_ORDER,
...
export function renderAutomatedSeaDropV3DeploymentManifestProposal(value) {
...
export async function readBoundedJsonFile(path, maximum = MAX_FILE_BYTES, label = "JSON") {
  ...
  try {
    ...
    try { return snapshot(JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)),
      ...
      if (error instanceof AutomationV3ManifestError) throw error;
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!["--artifact", "--git-commit", "--confirmations", "--release-root"].includes(flag)
      ...
      || Object.hasOwn(parsed, flag)) fail("INVALID_ARGUMENTS", "invalid V3 manifest arguments");
    parsed[flag] = value;
  }
...
function assertBroadcastPath(path, releaseRoot = projectRoot) {
  ...
  const expectedRoot = resolve(
    releaseRoot,
    "broadcast/DeployAutomatedSeaDropV3.s.sol/4663",
  );
  ...
  if (within.startsWith(`..${sep}`) || within === ".." || within.includes(sep)
    ...
    fail("INVALID_ARTIFACT_PATH", "broadcast artifact must be the fixed V3 chain-4663 run file");
  }
  return resolved;
}

async function loadCompiledArtifacts(releaseRoot = projectRoot) {
  return Object.fromEntries(await Promise.all(AUTOMATION_V3_DEPLOYMENT_ORDER.map(async (name) => [
    name,
...
async function main() {
  ...
  const foundryArtifactCommit = commit(broadcastArtifact.commit, "Foundry artifact commit",
    { foundry: true });
  const sourceProvenance = await verifyAutomationV3SourceProvenance({
    releaseGitCommit,
    foundryArtifactCommit,
    cwd: releaseRoot,
  });
  ...
  const proposal = await buildAutomatedSeaDropV3DeploymentManifestProposal({
    releaseGitCommit,
  ...
  process.stdout.write(renderAutomatedSeaDropV3DeploymentManifestProposal(proposal));
}

if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  main().catch((error) => {
    process.stderr.write(`${error instanceof AutomationV3ManifestError
      ? error.message : "AUTOMATION_V3_MANIFEST_FAILED: unexpected read-only failure"}\n`);
    process.exitCode = 1;
  });
}
