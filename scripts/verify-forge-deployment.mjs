...
try {
  ...
  const packet = JSON.parse(await readFile(path, 'utf8')), plan = packet.plan ?? packet;
  ...
  const evidence = await verifyForgeDeployment({ clients, plan, build, transactionHashes: args[2].slice('--transactions='.length).split(',') });
