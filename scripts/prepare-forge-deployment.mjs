...
import { loadForgeDeploymentBuild, buildForgeDeploymentPlan, assertForgeRuntime } from '../broker/src/v4/skill-forge/forge-deployment.mjs';
...
try {
  ...
  if (chains.some(chain => chain !== 4663)) throw Error('FORGE_DEPLOYMENT_WRONG_CHAIN');
  ...
  let plan = buildForgeDeploymentPlan({ build, administrator, nonce: String(nonces[0]), anchor });
  const observations = [];
  ...
  plan = buildForgeDeploymentPlan({ build, administrator, nonce: String(nonces[0]), anchor,
  ...
  console.log(JSON.stringify({ status: 'PREPARED_PAUSED_FORGE_DEPLOYMENT', observedAt: new Date().toISOString(), endpoints, plan,
    ...
    note: 'Recheck nonce, code, simulation and fees before each wallet request. The second transaction can only be simulated after deployment. This packet expires; it does not enable training or burns.' }, null, 2));
} catch (error) {
  console.error(JSON.stringify({ status: 'BLOCKED', phase, code: /^[A-Z_]+$/.test(error.message) ? error.message : 'FORGE_DEPLOYMENT_READ_OR_SIMULATION_FAILED',
