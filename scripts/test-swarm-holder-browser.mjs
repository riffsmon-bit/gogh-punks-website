...
try{
    ...
    if(message.method==='Fetch.requestPaused'){
      ...
      if(args[3]==='--local-url'&&u.pathname==='/broker-v2.js'){
        void readFile(`${repo}/site/broker-v2.js`,'utf8').then(source=>{
          // Inject only read fixtures into the local page. All existing production
          // modules and review gates still run; no provider method may be called.
          const mountNeedle = "swarmControl = mountSwarmSetup({ root: one('[data-swarm-panel]'),";
          assert.equal(source.split(mountNeedle).length, 2, 'Expected one guided setup integration');
          const mountFixture = mountNeedle + `
            readAccount: async (_provider, {owner, tokenId}) => {
              window.__swarmAccountChecks.push(tokenId);
              return {owner, tokenId, chainId:4663, created:true, account:'0x'+BigInt(tokenId).toString(16).padStart(40,'0')};
            },
            walletOptions: {client: {
              getSwarmWalletRecord: () => null,
              readSwarmWallet: async (_provider, {owner}) => {
                window.__swarmWalletReads++;
                return {owner, chainId:4663, created:true, vault:'0x'+'3'.repeat(40),
                  balanceWei:'1000000000000000', dependenciesVerified:true};
              },
              prepareSwarmWallet: async () => { throw Error('No funding transaction may be prepared by this read-only journey'); },
              submitSwarmWallet: async () => { throw Error('No wallet submission may occur in this read-only journey'); },
              recoverSwarmWallet: async () => { throw Error('No receipt is fabricated by this read-only journey'); },
            }},`;
          const guidedOffset = source.indexOf(mountNeedle);
          const guidedSource = source.slice(0,guidedOffset) + source.slice(guidedOffset).replace(
            'getProvider: () => window.__GOGH_WALLET_PROVIDER__,',
            'getProvider: () => window.__holderFixtureProvider,');
          const fixture=guidedSource.replace(mountNeedle, mountFixture).replace('const PREVIEW =','let PREVIEW =')+`\nwindow.__connectedHolderFixture=()=>{PREVIEW=false;window.__swarmAccountChecks=[];window.__swarmWalletReads=0;window.__walletCalls=[];window.__holderFixtureProvider={request:async request=>{window.__walletCalls.push(request.method);throw Error('Real wallet and RPC calls are forbidden in this local fixture');}};loadAgentAccountStatus=async()=>null;hydrateSelected=async()=>{};window.__signInCalls=0;ensureV2Session=async()=>{window.__signInCalls++;};jsonRequest=async(path,options)=>{if(path.endsWith('/chat')){window.__startRequest=JSON.parse(options.body);return{responseKind:'STRATEGY_DRAFT',draft:{intentHash:'0x'+'a'.repeat(64),intent:{...state.selected.strategy.intent,operatingMode:'AUTONOMOUS',totalMintLimit:window.__startRequest.message.includes('Keep hunting for up to 100')?100:5,expiration:new Date(Date.now()+(window.__startRequest.message.includes('Keep hunting for up to 100')?30:1)*86400000).toISOString(),allowedContracts:window.__startRequest.message.includes('Clear my collection target.')?[]:state.selected.strategy.intent.allowedContracts}}};}throw Error('Local connected fixture: no remote reads');};state.wallet={account:'0xc7f55ce6a7df9a79cc4a643a5081230f890c7aa6',chainId:4663,status:'owner'};state.punks=[{...previewPunks[0],tokenId:'93'},{...previewPunks[1],tokenId:'1135'}];state.selected=state.punks[0];state.selected.strategy={intent:{expectedOwner:'0xc7f55ce6a7df9a79cc4a643a5081230f890c7aa6',punkTokenId:'93',allowedContracts:['0xb73f1d1aee57410d537d87b656e98b9d3df5b213'],operatingMode:'ASK',dailyMintLimit:5,totalMintLimit:5,mintMode:'FREE_ONLY',maxGasPerMintWei:'500000000000000',minimumReserveWei:'10000000000000000',preferences:{prefer:['PIXEL'],avoid:[]},preferredSocialPlatforms:[]}};state.agentAccounts.set('93',{ok:true,owner:state.wallet.account,tokenId:'93',chainId:4663,receivedAt:Date.now(),runtime:{sessionActive:false},worker:{enabled:true},readiness:{setupAvailable:true},mission:{sessionId:'completed-93',status:'COMPLETED',completedMints:1,totalLimit:1,dailyLimit:1,opportunitiesChecked:4,checks:1,lastCheckedAt:'2026-09-12T23:53:01Z'}});window.__badgeFixture=()=>{const a=state.agentAccounts.get('93');a.receivedAt=Date.now();a.mission.sessionId='badge-check-'+Date.now();a.mission.status='COMPLETED';a.runtime.sessionActive=false;missionNotifications.observe({owner:state.wallet.account,tokenId:'93',account:a});};window.__refreshAgentFixture=()=>{state.agentAccounts.get('93').runtime.entryPointDeposit='10000000000000000';state.agentAccounts.get('93').runtime.nativeBalance='20000000000000000';state.agentAccounts.get('93').receivedAt=Date.now();renderAgentAccount();};window.__activationFixture=(kind)=>{const a=state.agentAccounts.get('93');a.receivedAt=Date.now();a.runtime={accountCreated:kind!=='setup',account:'0x'+'2'.repeat(40),owner:state.wallet.account,sessionActive:false,nativeBalance:kind==='funded'?'20000000000000000':'0',entryPointDeposit:'0'};a.readiness={setupAvailable:true,blockers:[]};state.localStrategy=null;renderActivationGuide();};window.__reserveFixture=()=>{const a=state.agentAccounts.get('93');a.runtime.nativeBalance=a.mission.intent.minimumReserveWei;renderMissionMonitor();};window.__gasFixture=(created)=>{const a=state.agentAccounts.get('93');a.runtime.accountCreated=created;a.runtime.account='0x'+'2'.repeat(40);a.runtime.nativeBalance='0';a.runtime.entryPointDeposit='0';renderAgentGasFunding(a);};window.__missionFixtureState=(kind)=>{const a=state.agentAccounts.get('93');a.receivedAt=Date.now();a.runtime={accountCreated:true,owner:state.wallet.account,account:'0x'+'2'.repeat(40),sessionActive:kind!=='complete',nativeBalance:'20000000000000000',entryPointDeposit:'0',session:{minimumNativeReserveWei:'10000000000000000'}};a.readiness.blockers=[];a.readiness.automaticExecutionReady=true;a.worker.enabled=kind!=='paused';a.mission=kind==='complete'?{status:'COMPLETED',completedMints:1,totalLimit:1,dailyLimit:1}:{account:a.runtime.account,intent:{expectedOwner:state.wallet.account,punkTokenId:'93',minimumReserveWei:'10000000000000000'},status:'ACTIVE',completedMints:0,totalLimit:5,dailyLimit:5,validAfter:new Date(Date.now()-60000).toISOString(),validUntil:new Date(Date.now()+86400000).toISOString(),lastCheckedAt:new Date(Date.now()-(kind==='stale'?240000:10000)).toISOString()};renderMissionMonitor();};window.__zeroPunkFixture=()=>{state.punks=[];state.selected=null;state.ownershipAccount=state.wallet.account;renderRoster();};renderRoster();renderSelected();publicPaidControl.selectionChanged();holderBurnInspectionControl.refresh();};`;
  ...
  const until=async expression=>{for(let i=0;i<150;i++){if(await evaluate(expression))return;await new Promise(resolve=>setTimeout(resolve,100));}throw Error(`UI_TIMEOUT:${expression} ${JSON.stringify(await evaluate(`({guide:document.querySelector('[data-swarm-setup-status]')?.textContent,wallet:document.querySelector('[data-swarm-setup-wallet] [data-swarm-wallet-status]')?.textContent,accountChecks:window.__swarmAccountChecks,walletReads:window.__swarmWalletReads,walletCalls:window.__walletCalls,fixtureProvider:!!window.__holderFixtureProvider})`))}`);};
  ...
  for(const [width,height]of [[1440,1000],[375,900],[320,900]]){
    ...
    if(args[3]==='--local-url') {
      ...
      await evaluate(`window.__missionFixtureState('active');window.__reserveFixture()`);
      assert.equal(await evaluate(`document.querySelector('[data-current-mission-status]').textContent`),'RESERVE REACHED');
      assert.equal(await evaluate(`document.querySelector('[data-current-mission-recall]').disabled`),false);
      assert.equal(await evaluate(`document.querySelector('[data-current-mission-recall]').getClientRects().length>0`),true);
      result.reserveRecallVisiblePassed=true;
      ...
      const planSignIns = await evaluate(`window.__signInCalls`);
      await evaluate(`document.querySelector('[data-open-swarm]').click();for(const n of document.querySelectorAll('[data-swarm-setup-plan-form] input[type=checkbox]')){n.checked=true;n.dispatchEvent(new Event('change'));}for(const n of document.querySelectorAll('[data-swarm-setup-plan-form] select')){if(['daily','total'].includes(n.name))n.value='5';}document.querySelector('[data-swarm-setup-plan-form] [name=fundingBudgetEth]').value='0.003';document.querySelector('[data-swarm-setup-plan-form]').requestSubmit()`);
      assert.match(await evaluate(`document.querySelector('[data-swarm-setup-summary]').textContent`),/Combined limits: 10\/day, 10 total\. 0\.003 ETH/);
      assert.match(await evaluate(`document.querySelector('[data-swarm-setup-content]').textContent`),/Punk #93: 0\.0015 ETH.*Punk #1135: 0\.0015 ETH/);
      assert.equal(await evaluate(`window.__signInCalls`),planSignIns);
      assert.deepEqual(await evaluate(`window.__swarmAccountChecks`),[]);
      assert.equal(await evaluate(`window.__swarmWalletReads`),0);
      await evaluate(`document.querySelector('[data-swarm-setup-check-wallets]').click()`);
      await until(`document.querySelector('[data-swarm-setup-to-funding]')&&!document.querySelector('[data-swarm-setup-to-funding]').disabled`);
      assert.deepEqual(await evaluate(`window.__swarmAccountChecks`),['93','1135']);
      await evaluate(`document.querySelector('[data-swarm-setup-to-funding]').click()`);
      await until(`!document.querySelector('[data-swarm-setup-wallet] [data-swarm-wallet-forms]').hidden&&!document.querySelector('[data-swarm-setup-wallet] [data-swarm-wallet-batch]').disabled`);
      const funding = await evaluate(`(()=>{const w=document.querySelector('[data-swarm-setup-wallet]');return {amount:w.querySelector('[data-swarm-wallet-batch-amount]').value,locked:w.querySelector('[data-swarm-wallet-batch-amount]').disabled,deposit:w.querySelector('[data-swarm-wallet-deposit-amount]').value,punks:[...w.querySelectorAll('[data-swarm-wallet-punks] input')].map(n=>({id:n.value,checked:n.checked,disabled:n.disabled})),consent:w.querySelector('[data-swarm-wallet-consent]').checked,reviewHidden:w.querySelector('[data-swarm-wallet-review]').hidden,missionBlocked:document.querySelector('[data-swarm-setup-to-missions]').disabled}})()`);
      assert.equal(funding.amount,'0.003');assert.equal(funding.locked,true);assert.equal(funding.deposit,'0.002');
      assert.deepEqual(funding.punks,[{id:'93',checked:true,disabled:true},{id:'1135',checked:true,disabled:true}]);
      assert.equal(funding.consent,false);assert.equal(funding.reviewHidden,true);assert.equal(funding.missionBlocked,true);
      assert.equal(await evaluate(`window.__swarmWalletReads`),1);
      assert.equal(await evaluate(`window.__signInCalls`),planSignIns);
      assert.deepEqual(await evaluate(`window.__walletCalls`),[]);
      const fundingShot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:true});const fundingFile='swarm-funding-'+width+'.png';await writeFile(join(output,fundingFile),Buffer.from(fundingShot.data,'base64'));result.screenshots.push(fundingFile);
      result.swarmFundingAllocationPassed=true;result.unconfirmedSwarmFundingBlocksMissions=true;
      // A separate, explicitly chosen existing-gas plan exercises real mission
      // review integration without inventing a confirmed funding receipt.
      await evaluate(`document.querySelector('[data-swarm-setup-close]').click()`);
      await evaluate(`(()=>{const f=document.querySelector('[data-swarm-setup-plan-form]');for(const n of f.querySelectorAll('input[type=checkbox]'))n.checked=true;f.elements.daily.value='5';f.elements.total.value='5';f.elements.fundingBudgetEth.value='';f.requestSubmit();document.querySelector('[data-swarm-setup-check-wallets]').click();})()`);
      await until(`document.querySelector('[data-swarm-setup-to-funding]')&&!document.querySelector('[data-swarm-setup-to-funding]').disabled`);
      await evaluate(`document.querySelector('[data-swarm-setup-to-funding]').click()`);
      await until(`document.querySelector('[data-swarm-setup-to-missions]')&&!document.querySelector('[data-swarm-setup-to-missions]').disabled`);
      assert.match(await evaluate(`document.querySelector('[data-swarm-setup-content]').textContent`),/No deposit requested/);
      assert.equal(await evaluate(`window.__swarmWalletReads`),1);
      await evaluate(`document.querySelector('[data-swarm-setup-to-missions]').click()`);
      await until(`document.querySelector('[data-swarm-setup-next-mission]')&&!document.querySelector('[data-swarm-setup-next-mission]').disabled`);
      assert.equal(await evaluate(`window.__signInCalls`),planSignIns);
      assert.deepEqual(await evaluate(`window.__walletCalls`),[]);
      ...
      assert.equal(await evaluate(`document.querySelector('[data-confirmation-dialog]').open`),false);
      await evaluate(`window.__activationFixture('gas');window.__gasFixture(true)`);
      assert.equal(await evaluate(`document.querySelector('[data-agent-next-mission]').disabled`),true);
      assert.equal(await evaluate(`document.querySelector('[data-v2-tab=fund]').getAttribute('aria-selected')`),'true');
      assert.equal(await evaluate(`document.querySelector('[data-agent-gas-form]').getClientRects().length>0`),true);
      assert.equal(await evaluate(`document.querySelector('[data-confirmation-dialog]').open`),false);
      ...
      const signInBefore=await evaluate(`window.__signInCalls`);
      await evaluate(`document.querySelector('[data-swarm-setup-next-mission]').click()`);
      await until(`document.querySelector('[data-swarm-review-dialog]').open`);
      assert.equal(await evaluate(`window.__signInCalls`),signInBefore);
      assert.equal(await evaluate(`document.querySelector('[data-confirmation-dialog]').open`),false);
      assert.match(await evaluate(`document.querySelector('[data-swarm-review-grid]').textContent`),/FREE ONLY/);
      assert.equal(await evaluate(`document.querySelector('[data-swarm-review-dialog]').scrollWidth<=document.querySelector('[data-swarm-review-dialog]').clientWidth`),true);
      const reviewShot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});const reviewFile='swarm-review-'+width+'.png';await writeFile(join(output,reviewFile),Buffer.from(reviewShot.data,'base64'));result.screenshots.push(reviewFile);
      await evaluate(`document.querySelector('[data-swarm-review-cancel]').click()`);
      await until(`!document.querySelector('[data-swarm-review-dialog]').open&&!document.querySelector('[data-swarm-setup-next-mission]').disabled`);
      assert.equal(await evaluate(`window.__signInCalls`),signInBefore);
      assert.deepEqual(await evaluate(`window.__walletCalls`),[]);
      await evaluate(`document.querySelector('[data-swarm-setup-next-mission]').click()`);
      await until(`document.querySelector('[data-swarm-review-dialog]').open`);
      await evaluate(`document.querySelector('[data-swarm-review-continue]').click()`);
      await until(`document.querySelector('[data-confirmation-dialog]').open`);
      ...
      await evaluate(`document.querySelector('[data-confirmation-dialog]').close()`);
      await until(`document.querySelector('[data-swarm-panel]').getAttribute('aria-busy')==='false'`);
      assert.match(await evaluate(`document.querySelector('[data-swarm-setup-content]').textContent`),/Punk #93 · Review opened · not approved.*Punk #1135 · Ready for rules review/);
      assert.equal(await evaluate(`window.__signInCalls`),signInBefore+1);
      assert.deepEqual(await evaluate(`window.__walletCalls`),[]);
      result.swarmReviewPassed=true;result.passiveSwarmReviewPassed=true;
      ...
      await evaluate(`document.querySelector('[data-swarm-setup-close]').click();document.querySelector('[data-swarm-details]').open=false`);
    ...
    if(args[3]==='--local-url') {
      await evaluate(`window.__zeroPunkFixture();document.querySelector('[data-swarm-wallet-details]').open=true`);
      const ownerWallet=await evaluate(`(()=>{const panel=document.querySelector('[data-swarm-wallet-panel]');return{selectedHidden:document.querySelector('[data-selected-stage]').hidden,welcomeHidden:document.querySelector('[data-broker-welcome]').hidden,visible:panel.getClientRects().length>0,insideSelected:!!panel.closest('[data-selected-stage]'),canCheck:!panel.querySelector('[data-swarm-wallet-check]').disabled,rosterCount:document.querySelector('[data-roster-count]').textContent}})()`);
      assert.equal(ownerWallet.selectedHidden,true);assert.equal(ownerWallet.welcomeHidden,true);assert.equal(ownerWallet.visible,true);assert.equal(ownerWallet.insideSelected,false);assert.equal(ownerWallet.canCheck,true);assert.equal(ownerWallet.rosterCount,'0');
      await evaluate(`document.querySelector('[data-swarm-wallet-details]').scrollIntoView({block:'start'})`);
      const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:true});const file='zero-punk-swarm-wallet-'+width+'.png';await writeFile(join(output,file),Buffer.from(shot.data,'base64'));result.screenshots.push(file);
      result.zeroPunkOwnerWalletReachable=true;
    }
    result.viewports.push({width,height,states});
  }
  for(const path of ['/broker-swarm-setup.js','/broker-swarm-setup-state.js','/swarm-wallet-rpc.js','/swarm-wallet-errors.js','/punk-agent-creation.js','/punk-agent-creation-panel.js','/swarm-wallet-release.js','/swarm-wallet-client.js','/swarm-wallet-panel.js','/broker/v2/index.html','/broker-v2.js','/broker-mission-status.js','/forge-skill-admin-panel.js','/broker-persistent-watch-mount.js','/broker-persistent-watch.js','/broker-v2.css','/broker-persistent-watch.css','/broker-agent-options.js','/directed-paid-public-panel.js','/erc20-withdraw-panel.js','/forge-holder-inspection-panel.js','/guide/index.html','/broker-action-feedback.js','/broker-mission-status.js','/forge-paid-release.js','/forge-paid-panel.js','/forge-paid-wallet.js','/broker-swarm.js','/broker-swarm-review.js','/broker-swarm-funding.js','/broker-activation-status.js','/broker-mission-notifications.js','/broker-feature-help.js','/forge-selected-burn-panel.js','/forge-holder-inspection-panel.js']){
