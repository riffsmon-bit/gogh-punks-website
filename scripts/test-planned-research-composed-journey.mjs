...
const startedAt = Date.now(), steps = [], directory = await mkdtemp(join(tmpdir(), 'gogh-planned-skill-journey-'));
...
try {
  ...
  await write(release.registry, 'setEmergencyControls', [false, (2n ** 256n - 1n) ^ 77n]);
  const selection = PLANNED_RESEARCH_SELECTION;
  ...
  mark('BURN_COPIES_FOR_THREE_CREDITS');
  ...
  assert.equal(await progress('trainingCredits'), creditBefore + 3n);
  assert.equal(await read(release.collection, 'totalSupply'), supplyBefore - 3n);
  ...
  const fetchImpl = async (url, options) => {
    ...
    assert.ok(new URL(url).pathname.startsWith('/api/v2/'));
  ...
  const research = createResearchSkillRuntime({ client, packages, readState, apiKey, fetchImpl });
  const operations = [
    ['rank_observed_listings', { slug: 'gogh-punks-255843210', contract: release.collection, limit: 5 }],
    ['research_collection', { contract: release.collection, tokenIds: ['93', '96', '97'] }],
    ['classify_collection', { contract: release.collection, tokenIds: ['93', '96', '97'], preferredStyles: ['PIXEL_ART'] }],
  ];
  const results = [];
  ...
  for (const [index, pack] of packages.entries()) {
    ...
    if (name === 'rank_observed_listings') {
      assert.equal(result.schema, 'GOGH_OBSERVED_LISTING_RANKS_V1');
      assert.equal(result.coverage.status, 'BOUNDED_SAMPLE'); assert.ok(result.rankedGroups.length > 0);
      assert.equal(result.collectionFloor, null); assert.equal(result.collectionFloorVerified, false);
    }
    if (name === 'research_collection') {
      assert.equal(result.schema, 'GOGH_COLLECTION_RESEARCH_V1');
      assert.equal(result.coverage.observedCount, 3);
      assert.equal(result.coverage.collectionComplete, false);
    }
    if (name === 'classify_collection') {
      assert.equal(result.schema, 'GOGH_DECLARED_ART_STYLE_MATCHES_V1');
      assert.equal(result.coverage.observedCount, 3);
    ...
    results.push({ skill: pack.manifest.name, version: pack.manifest.version, key, tool: name, status: 'PASS',
      resultSchema: result.schema, coverage: result.coverage,
      observedOrders: result.rankedGroups?.reduce((sum, group) => sum + group.listings.length, 0) ?? null,
      recognizedTokenCount: result.recognizedTokenCount ?? null, unknownTokenCount: result.unknownTokenCount ?? null,
      evidenceHash: result.evidenceHash, manifestHash: pack.manifestHash, instructionHash: pack.instructionHash,
      implementationSha256: pack.manifest.implementationSha256 });
  ...
  const slots = await progress('unlockedSlots');
  ...
  assert.equal(buyerResult.collectionFloor, null); assert.ok(buyerResult.rankedGroups.length > 0);
  await assert.rejects(train(3, ZERO, owner));
  await train(3, ZERO, buyer); await fresh();
  await assert.rejects(research.call({ tokenId, owner: buyer, name: operations[0][0], arguments: operations[0][1] }), /SKILL_TOOL_DENIED/);
  ...
  const result = { schema: 'GOGH_PLANNED_RESEARCH_COMPOSED_JOURNEY_V1', status: 'PASS', checkedAt: new Date().toISOString(),
    ...
    targetTokenId: tokenId, copiedSacrifices: sources, copiedCreditGain: 3, creditsSpentLearning: 3,
    ...
    results, transfer: { transactionHash: transfer, allLearnedSkillsPersist: true, slotsPersist: true,
      loadoutPersists: true, oldOwnerDenied: true, newOwnerResearchWorks: true, buyerUnequipDenies: true },
    ...
    limitations: ['Copied-chain fixture administration only; no public skill registration or READY transition.',
      ...
      'Floor Hunter uses actual read-only OpenSea listing observations, never a verified collection floor.',
      'Collection and art adapters read original contract/metadata state copied at the fork anchor.',
      'Art Curator matches declared labels only and does not classify images; unknown output is preserved.',
      'No wallet transaction or owner-policy mutation is returned or performed.',
      'Original token transfer continuity retains its separate restrictions.'], steps };
  await writeFile(new URL('../docs/v2-hardening/planned-research-composed-evidence.json', import.meta.url), JSON.stringify(result, null, 2) + '\n');
  ...
  console.error(JSON.stringify({ status: 'FAIL', phase: stage, type: error.name, code: error.code ?? null,
    ...
    frames: error.stack?.split('\n').filter(line => line.includes('test-planned-research') || line.includes('skill-forge')).slice(0, 4) }));
  process.exitCode = 1;
} finally {
  enabled = false;
