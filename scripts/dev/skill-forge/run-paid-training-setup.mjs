...
import { loadPaidTrainingDeployment, verifyPaidTrainingDependencies, verifyPaidTrainingDeployment } from './paid-training-deployment.mjs';

if (process.argv.length !== 3 || !['--live-owner-wallet','--enable-reviewed-purchases'].includes(process.argv[2])) throw Error('Requires explicit deployment or purchase-enable mode');
const enabling = process.argv[2] === '--enable-reviewed-purchases';
const { artifact: paid, release, configuration } = await loadPaidTrainingDeployment({ allowDeployed: enabling });
const owner = configuration.guardian;
let archive;
...
const steps = enabling ? [{ action:'ENABLE_PAID_TRAINING', label:'Enable 0.0005 ETH Training Credit purchases', to:release.extension,
  data:encodeFunctionData({abi:paid.abi,functionName:'setPurchasesPaused',args:[false]}) }]
  : [{ action: 'DEPLOY_PAID_TRAINING', label: 'Deploy paid Training Credits · purchases remain paused', to: null,
    data: encodeDeployData({ abi: paid.abi, bytecode: paid.bytecode.object, args: [configuration, release.skills.map(s => s.key)] }) }];
const binding = setupDigest({ owner, steps, release });
const journal = openSetupReviewJournal({ path: join(homedir(), '.gogh-punks', enabling ? 'paid-training-enable.sqlite' : 'paid-training-setup.sqlite'), binding });
const same = (a,b) => typeof a === 'string' && typeof b === 'string' && a.toLowerCase() === b.toLowerCase();
const valid = (v,code) => { if (!v) throw Error(code); }, hex = v => '0x' + BigInt(v).toString(16);
const config = { owner, chainId:4663, priceWei:release.priceWei, treasury:release.treasury, skills:release.skills.map(s=>s.name),
  walletAuthority:'NONE', steps:steps.map(({action,label,to})=>({action,label,to})), burnEnabled:false,
  completionMessage:enabling ? 'Purchase enablement verified by both providers. The website release and first owner purchase still need verification.' : 'Paid-training deployment receipt verified by both providers. Purchases remain paused until deployment verification and the separate release review are complete.' };
async function context() {
  ...
  for (const client of clients) {
    ...
    await verifyPaidTrainingDependencies({ client, release, blockNumber:block.number });
    if (enabling) await verifyPaidTrainingDeployment({client,artifact:paid,release,address:release.extension,owner,blockNumber:block.number});
  }
  return block;
}
...
async function inspect(hash,review) {
  ...
  for(const c of clients) {
    ...
    if (r.status === 'success') await verifyPaidTrainingDeployment({ client:c, artifact:paid, release, address:expected.to ?? review.predictedAddress, owner, blockNumber:r.blockNumber, purchasesPaused:!enabling });
...
const server=createServer(async(req,res)=>{
  ...
  try {
    if(req.method==='GET') {
      ...
      const files={'/':[enabling?'paid-training-enable.html':'paid-training-setup.html','text/html'],'/setup.js':['selected-launch-setup.js','text/javascript'],'/setup.css':['owner-deployment.css','text/css']};
...
const port=enabling?64349:64348;
