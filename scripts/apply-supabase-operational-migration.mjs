...
try {
  ...
  const already = await client.query(
    `SELECT to_regclass('public.gogh_broker_punk_jobs') IS NOT NULL AS jobs,
            ...
            to_regclass('public.gogh_broker_punk_priority_attempts') IS NOT NULL AS priority_attempts`,
  );
  console.log(JSON.stringify({
    ...
    priorityAttemptTablePresent: already.rows[0]?.priority_attempts === true,
  }));
  if (APPLY) {
    ...
    const verified = await client.query(
      ...
      [["gogh_broker_punk_state", "gogh_broker_punk_jobs", "gogh_broker_worker_runs",
        ...
        "gogh_broker_punk_priority_attempts"]],
    );
    console.log(JSON.stringify({
      migrations: MIGRATIONS,
      applied: true,
