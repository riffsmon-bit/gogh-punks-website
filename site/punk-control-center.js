...
import { fetchPrepaidAgentGasStatus } from "./prepaid-agent-gas.js";
...
function renderPrepaidAgentGas() {
  ...
  if (card) card.hidden = !state.owned;
  for (const element of queryAll("[data-prepaid-agent-token]")) {
  ...
  setText("[data-prepaid-agent-refund]", status?.refund?.eligible
    ? `${formatNative(status.refund.availableWei)} eligible · manifest review required`
    : status?.session ? "Available after this session settles" : "No refundable balance verified");
  ...
  setText("[data-prepaid-agent-session]", session
    ...
    : "Not started");
  const sessionHistory = query("[data-prepaid-agent-session-history]");
  if (sessionHistory) {
    const rows = status?.sessionHistory ?? [];
    sessionHistory.replaceChildren(...(rows.length === 0 ? ["No priority sessions recorded."]
      : rows.map((item) => `${item.state} · ${item.completedMints}/${item.requestedMints} mints · ${new Date(item.startsAt).toLocaleString()}${item.lastResult ? ` · ${item.lastResult}` : ""}`))
      .map((value) => {
