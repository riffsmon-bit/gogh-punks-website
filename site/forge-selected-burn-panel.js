...
export function createSelectedBurnPanel({root,getSelection,ensureSession,request,getProvider=()=>window.__GOGH_WALLET_PROVIDER__}) {
  if(!root)return null;let envelope=null,busy=false,key='',sequence=0,message='';
  ...
  const statusText=r=>!r?'Check the selected Punks before preparing a transaction.':({PREPARED:'Review ready. No wallet request has been made.',
    WALLET_REQUESTED:'Wallet confirmation was requested. Recover or recheck the original transaction.',
  ...
  function selectionChanged(){const s=getSelection(),next=`${s?.owner}:${s?.tokenId}:${s?.chainId}:${s?.preview}`;
    if(next!==key){key=next;++sequence;busy=false;envelope=null;message='';render();}}
  ...
  const prepare=action=>work(async(selected,current)=>{const result=validateSelectedBurnEnvelope(await api({operation:'prepare',action}),selected);
    if(current()){envelope=result;message=statusText(result.record);}});
  ...
  function render(){root.replaceChildren();root.hidden=!available();if(!available())return;
    ...
    if(r?.status==='PREPARED'){
      const v=r.review;root.append(el('h4',v.action==='ENABLE_FORGE'?'ENABLE FORGE':v.action==='APPROVE'?'APPROVE ONLY #1753':'CONFIRM THE PERMANENT BURN'),
      ...
      if(v.action!=='ENABLE_FORGE'){
        root.append(el('p','Fresh checks found no incoming standard token transfers, ETH, WETH, Agent gas deposits or application obligations in #1753’s four wallets. Nonstandard assets and off-chain obligations still require your review.'));
        const label=el('label');checked=el('input');checked.type='checkbox';label.append(checked,document.createTextNode('I reviewed #1753’s wallets and have no remaining assets, pending transactions or obligations to preserve.'));root.append(label);
      }
      if(v.action==='BURN'){const label=el('label','Type BURN 1753');confirmation=el('input');confirmation.autocomplete='off';confirmation.maxLength=9;label.append(confirmation);root.append(label);}
      const attempted=saved(v.intentId)?.attempted===true;
      button(root,'CONFIRM IN WALLET',()=>{const accepted=checked?.checked??false,text=confirmation?.value??'';
    ...
    if(r?.status==='WALLET_REQUESTED'){
      const label=el('label','Transaction hash from wallet activity'),input=el('input');input.maxLength=66;input.placeholder='0x…';input.value=saved(r.review.intentId)?.hash??r.reportedHash??'';label.append(input);root.append(label);
  ...
  selectionChanged();return {selectionChanged,destroy(){++sequence;}};
}
