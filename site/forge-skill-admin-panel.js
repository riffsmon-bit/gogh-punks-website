...
const TERMINAL=new Set(['CONFIRMED','REVERTED','REPLACED','CANCELLED']);
...
export function validateSkillAdminCancellationReview(saved,record,owner,now=Date.now()) {
  const p=saved?.preparation,t=p?.transaction;
  if(!saved||saved.status!=='PREPARED'||!UUID.test(saved.id)||saved.parentId!==record?.id||saved.administrator!==owner
    ||saved.registry!==REGISTRY||record.administrator!==owner||!['WALLET_REQUESTED','SUBMITTED'].includes(record.status)
    ||p?.schema!=='GOGH_SKILL_ADMIN_NONCE_CANCELLATION_V1'||p.parentId!==record.id||p.administrator!==owner
    ||p.registry!==REGISTRY||p.chainId!==4663||p.walletConfirmationRequired!==true||p.registryActionRepeated!==false||p.assetValueWei!=='0'
    ||!['0x','0xef010063c0c19a282a1b52b07dd5a65b58948a07dae32b'].includes(p.accountCode)
    ||!Number.isSafeInteger(p.expiresAt)||p.expiresAt<=now+5000||p.expiresAt>now+65000
    ||!t||t.from!==owner||t.to!==owner||t.value!=='0x0'||t.data!=='0x'||t.chainId!=='0x1237'
    ||t.nonce!==record.preparation.transaction.nonce||p.originalNonce!==t.nonce
    ||Object.keys(t).sort().join(',')!=='chainId,data,from,gas,gasPrice,nonce,to,value'
    ||!['nonce','gas','gasPrice'].every(key=>/^0x(?:0|[1-9a-f][0-9a-f]*)$/i.test(t[key]))
    ||BigInt(t.gas)<=0n||BigInt(t.gas)>120000n||BigInt(t.gasPrice)<=0n
    ||BigInt(t.gas)*BigInt(t.gasPrice)!==BigInt(p.maximumNetworkFeeWei)||BigInt(p.maximumNetworkFeeWei)>100000000000000n)
    throw Error('The zero-value cancellation does not match the reserved nonce. Recheck the saved review.');
  return structuredClone(t);
}
...
export function createForgeSkillAdminPanel({root,getSelection,ensureSession,request,
  ...
  let identity='',generation=0,busy=false,snapshot=null,record=null,cancellation=null,journal=null,message='';
  ...
  function update(){const next=currentIdentity();if(next===identity)return;identity=next;++generation;busy=false;snapshot=null;record=null;cancellation=null;journal=null;message='';
  ...
  async function work(fn){if(busy)return;const owner=identity,ticket=++generation,current=()=>identity===owner&&currentIdentity()===owner&&ticket===generation;
    busy=true;render();try{await fn(owner,current);}catch(error){if(current())message=Number(error?.code)===4001?'Wallet confirmation was rejected. The saved registry request is still held. Recheck it, then review a zero-value nonce cancellation to continue safely.':error?.message??'The review could not be checked. Recheck the original transaction.';}
  ...
  function accept(payload,owner){if(payload?.ok!==true||payload.snapshot?.administrator?.toLowerCase()!==owner
    ...
    snapshot=payload.snapshot;record=payload.record;cancellation=payload.cancellation??null;
    if(cancellation&&(cancellation.administrator!==owner||cancellation.parentId!==record?.id))throw Error('Saved cancellation belongs to another review.');
  ...
  async function refresh(owner,current){let payload;try{payload=await api(null,journal?.id);}catch(error){if(current())snapshot=null;throw error;}if(!current())return;
    accept(payload,owner);
    const knownHash=journal?.transactionHash??record?.recoveryHash??record?.transactionHash;
    if(record&&knownHash&&!TERMINAL.has(record.status)){
      await api({operation:'recover',id:record.id,transactionHash:knownHash});if(!current())return;
      try{payload=await api(null,record.id);}catch(error){if(current())snapshot=null;throw error;}if(!current())return;accept(payload,owner);
    }
    message=record?({PREPARED:'Review ready. Check the skill, action and network fee before opening your wallet.',
      WALLET_REQUESTED:'A wallet confirmation was reserved. Recover its hash, or review a zero-value cancellation of its reserved nonce.',
      ...
      REPLACED:'The reserved nonce was consumed by a confirmed replacement. The saved registry request cannot execute. Recheck skills to continue.',
  ...
  const prepareCancellation=()=>work(async(owner,current)=>{
    await ensureSession();if(!current())return;
    const requestKey=crypto.randomUUID();persist({...journal,id:record.id,cancellationRequestKey:requestKey,cancellationAttempted:false},owner);
    const payload=await api({operation:'prepare_cancel',id:record.id,requestKey});if(!current())return;
    if(!payload.cancellation||payload.cancellation.administrator!==owner)throw Error('The cancellation review could not be saved. Recheck it.');
    persist({...journal,cancellationId:payload.cancellation.id},owner);
    await refresh(owner,current);
    if(current())message='Cancellation review ready. This is a zero-value self transaction with a network fee. The original registry step may confirm first.';
  });
  const confirmCancellation=()=>work(async(owner,current)=>{
    await ensureSession();if(!current())return;
    if(!cancellation||journal?.cancellationAttempted&&journal.cancellationId===cancellation.id)throw Error('Recheck the saved cancellation or prepare a new review.');
    const tx=validateSkillAdminCancellationReview(cancellation,record,owner),provider=getProvider(),saved={...journal,id:record.id},original=cancellation;
    const checkWallet=async()=>{const[accounts,chain]=await Promise.all([provider.request({method:'eth_accounts'}),provider.request({method:'eth_chainId'})]);
      if(!current()||accounts?.[0]?.toLowerCase()!==owner||BigInt(chain)!==4663n)throw Error('Wallet selection changed. Recheck the saved cancellation.');};
    if(!provider?.request)throw Error('Connect the administrator wallet first.');await checkWallet();
    persist({...saved,cancellationId:original.id,cancellationAttempted:true},owner);
    const claim=await api({operation:'claim_cancel',id:record.id,cancellationId:original.id,revision:original.revision,reviewHash:original.reviewHash});
    if(!current())return;
    if(claim?.cancellation?.status!=='WALLET_REQUESTED'||!claim.transaction||Object.keys(claim.transaction).length!==Object.keys(tx).length
      ||Object.keys(tx).some(key=>claim.transaction[key]!==tx[key]))throw Error('The cancellation claim could not be verified. Recheck its saved state.');
    await checkWallet();if(Date.now()>=original.preparation.expiresAt)throw Error('The cancellation review expired. Prepare another cancellation review.');
    const hash=await provider.request({method:'eth_sendTransaction',params:[tx]});
    if(!HASH.test(hash??''))throw Error('The wallet did not return a hash. Recover the cancellation from wallet activity.');
    persist({...saved,cancellationId:original.id,cancellationAttempted:true,transactionHash:hash.toLowerCase()},owner);
    if(current())await refresh(owner,current);
  });
  ...
  function render(){root.replaceChildren();root.append(element('h3','SKILL RELEASE ADMINISTRATION'));
    ...
    if(record&&!TERMINAL.has(record.status)){
      ...
      button(root,'REVIEW NONCE CANCELLATION',prepareCancellation);
      if(cancellation?.status==='PREPARED'){
        root.append(element('h4','Cancel the reserved nonce'),element('p',`Send 0 ETH to your own wallet. Maximum network fee: ${eth(cancellation.preparation.maximumNetworkFeeWei)}. This does not repeat the registry step.`),
          element('p','The original request may confirm first. Wait for the confirmed result before another registry review.'));
        button(root,'CONFIRM CANCELLATION IN WALLET',confirmCancellation,journal?.cancellationAttempted&&journal.cancellationId===cancellation.id||Date.now()+5000>=cancellation.preparation.expiresAt);
      }
      const label=element('label','Original or replacement transaction hash from wallet activity'),input=element('input');input.type='text';input.maxLength=66;input.placeholder='0x…';input.value=journal?.transactionHash??record.recoveryHash??record.transactionHash??'';label.append(input);root.append(label);
      button(root,'RECOVER TRANSACTION',()=>work(async(owner,current)=>{const hash=input.value.trim().toLowerCase();if(!HASH.test(hash))throw Error('Enter the full transaction hash from your wallet.');
