...
export function punkChatAction(message) {
  ...
  if (/^(?:please\s+)?(?:call (?:my |the )?punk back|come back|return home|pause(?: (?:my |the )?(?:punk|mission|agent))?|stop (?:the mission|minting|scouting))[.!]*$/i.test(text)) return { kind: "RECALL" };
