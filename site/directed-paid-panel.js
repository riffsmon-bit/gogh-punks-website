...
export function createDirectedPaidPanel({root,getSelection,ensureSession,request,getProvider=()=>window.__GOGH_WALLET_PROVIDER__}){
 if(!root)return null;let envelope=null,key='',sequence=0,busy=false,message='',maximumPriceWei=null;
 ...
 const recordText=r=>!r?'Request a directed paid mint in chat, or review one Peppies World mint here.':({
  PREPARED:'Quote ready. Review the amounts, then confirm once in your wallet.',WALLET_REQUESTED:'Wallet confirmation requested. Recheck or recover the original transaction.',
  CONFIRMED:r.review.action==='AUTHORIZE'?'Budget confirmed. The worker will check and execute this mint; recheck for its result.':r.review.action==='CANCEL_MISSION'?'Mission cancelled. Withdraw the refund below.':'Refund returned to your wallet.',
  REVERTED:'The wallet transaction reverted. Recheck before preparing another review.',CANCELLED:'Unsent review cancelled.',DECLINED:'Wallet confirmation declined. You can prepare a fresh review.'})[r.status];
 ...
 async function refresh(selection,current){
  ...
  if(current())message=recordText(envelope.record);
 }
 ...
 const prepare=action=>work(async(selection,current)=>{const result=validatePaidEnvelope(await api({operation:'prepare',action,maximumPriceWei:action==='AUTHORIZE'?maximumPriceWei:null}),selection);
  if(current()){envelope=result;message=recordText(result.record);}});
 ...
 function render(){root.replaceChildren();root.hidden=!available();if(!available())return;
  ...
  const status=el('p',matchingExecution&&envelope.execution.status==='COMPLETED'?'Mint complete. Delivery was verified by both chain providers.':message||recordText(envelope?.record));status.setAttribute('role','status');status.setAttribute('aria-live','polite');root.append(status);
  ...
  if(record?.status==='PREPARED'){
   ...
   button(v.action==='AUTHORIZE'?'CONFIRM MINT BUDGET IN WALLET':'CONFIRM IN WALLET',()=>work(async(selection,current)=>{
 ...
 selectionChanged();return {selectionChanged,openDraft,destroy(){++sequence;}};
}
