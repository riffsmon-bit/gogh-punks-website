export const FEATURE_HELP = Object.freeze({
  talk: {
    ...
    steps: ['For a new Agent wallet, open Fund first: create its wallet, then add gas. Creating the wallet does not start a mission.',
  ...
  strategy: {
    ...
    steps: ['Read the saved art preferences, price limit, network-fee limit, collection limits and reserve.',
      ...
      'For a new mission, create the Agent wallet and fund it first. Start mission requests the separate, bounded minting permission after you review the rules. Pause or recall stops new eligible work; submitted transactions still need confirmation.'],
    cost: 'Reviewing rules is free. Wallet creation, permission changes and transactions may require network fees shown by your wallet.',
  },
  fund: {
    ...
    steps: ['Choose the Punk Wallet or Agent Account. They are different addresses; check the destination.',
      'If the Agent wallet does not exist yet, review its creation and confirm in your connected wallet. This creates only the wallet; it does not grant minting permission or start a mission.',
      'Enter an amount, review the resulting balance and network fee, then confirm in your wallet.',
      'After funding a new Agent wallet, open Actions, set its rules and choose Start mission. Review and confirm the separate mission permission when you are ready.',
      'An existing Agent wallet can receive gas without Recall. If its mission permission is still active, adding gas can let it resume collecting within those approved rules. Recall first only if you want to stop that permission.',
    ...
    cost: 'Your connected wallet pays the network fee for wallet creation and funding; keep some ETH there. Funding alone does not activate a new mission or grant minting permission.',
  },
  collection: {
